...
class ProductMarginStdPrice(osv.osv_memory):
    _name = 'product.margin.std.price'
    _description = 'Sales Margins (cost price)'
    _columns = {
    ...
    def action_open_window(self, cr, uid, ids, context=None):
        ...
        cr.execute(
            ...
            ('product.margin.std.price.graph', 'graph')
        )
        view_res3 = cr.fetchone()[0]
        cr.execute(
            'select id, name from ir_ui_view where name=%s and type=%s',
            ('product.margin.std.price.form', 'form')
        )
        view_res2 = cr.fetchone()[0]
        cr.execute(
            'select id, name from ir_ui_view where name=%s and type=%s',
            ('product.margin.std.price.tree', 'tree')
        )
